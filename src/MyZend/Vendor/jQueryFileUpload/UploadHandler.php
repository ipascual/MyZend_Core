<?php
namespace MyZend\Vendor\jQueryFileUpload;
...
class UploadHandler
{
    ...
    protected function trim_file_name($name, $type, $index, $content_range) {
		/*
		 * MyZend every filename will have an UUID
		 * Windows
		 * 	image/x-png
		 * 	image/gif
		 * 	image/pjpeg
		 ...
		 * 
		 */
		$new_file_name = new \MongoId();
		$new_file_name = $new_file_name->{'$id'};
		 		 
		//Image
		preg_match('/^image\/(gif|jpe?g|png|x-png|pjpeg)/', $type, $matches);
		if(isset($matches[1])) {
			if($matches[1] == "x-png") {
				$matches[1] = "png";
			}
			...
			if(isset($matches[1])) {
				$new_file_name = $new_file_name . "." . $matches[1];
			}
		}
